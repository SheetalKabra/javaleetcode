...
//create a one distance array, which is having src to src value is 0, and for rest initially we have infinite
        Integer[] distance = new Integer[A];
        Arrays.fill(distance, Integer.MAX_VALUE);
        distance[C] = 0;

//now create a priority queue here, which will store in the form of pairs
        PriorityQueue<Pair> pq = new PriorityQueue<>(Comparator.comparingInt(x -> x.distance));
//initially add (0, src) which is (0,C)
        pq.add(new Pair(0, C));
//How and when values will add into pq: every time you get a better distance from previous one, then just go and insert into the pq
