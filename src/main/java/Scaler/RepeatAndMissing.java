...
public class RepeatAndMissing {
    public ArrayList<Integer> repeatedNumber(final List<Integer> A) {
        /** using xor
         take another array B of values 1 to N
         Do the xor of all the values of given array A and B
         You will get the left value with missing and repeated
         Now find out the set BIT from the rightmost
         And then create 2 variables having BIT = set, and BIT= unset
         */
        int N = A.size();
        int xorAll = 0;
        //do the xor aff all elements
        for(int i=1; i<=N; i++){
            xorAll = xorAll^A.get(i-1)^i;
        }
        System.out.println(xorAll);
        //find the set bit from the rightmost
        int temp = xorAll;
        int bit = 0;
        while(((temp)&(1<<bit)) == 0){
            bit++;
        }
        //now then create 2 variables having BIT = set, and BIT= unset
        int unset = 0;
        int set = 0;
        for(int i=1; i<=N; i++){
            if((A.get(i-1)&(1<<bit)) != 0 ){
                //if BIT is set
                set = set^A.get(i-1);
            }else{
                // BIT is unset
                unset = unset^A.get(i-1);
            }
        ...
        //now check for set and unset bit into the given array
        //if set^A.get(i) == 0, then repeated = set, else unset
        ...
        if(isRepeat){
            return new ArrayList<Integer>(Arrays.asList(set, unset));
        }else{
            return new ArrayList<Integer>(Arrays.asList(unset, set));
        }
