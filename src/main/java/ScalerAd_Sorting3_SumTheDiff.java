...
//        System.out.println(solve(new ArrayList<>(Arrays.asList(5,5,3,4))));
    ...
    public static int solve(ArrayList<Integer> A) {
        ...
        long sub = 0;
        for(int i=0; i<N; i++){
            sum = (sum%m + (A.get(i)*pow(2,i, m))%m)%m;
        }
        for(int i=0; i<N; i++){
            sub = (sub%m + (A.get(i)*pow(2, N-i-1, m))%m)%m;
        }
        int ans = (int)(sum - sub);
        if(ans < 0){
            return ans+m;
